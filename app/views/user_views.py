...
def create_user():
    try:
        ...
        new_user = UsersModel(name=name,email=email,status="ativo")
...
def delete_user():
    try:
        ...
        found_user.status = "desabilitado"
        session.add(found_user)
        session.commit()

        return "Usuario deletado",HTTPStatus.OK
    except Exception:
        return "Usuario Não existe",HTTPStatus.BAD_REQUEST 
